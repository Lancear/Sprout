<script>
  ...
  $: filteredIdeas = allIdeasPromise.then(ideas => ideas.filter((idea) => {
    if (selectedState && idea.growth !== selectedState) return false;
...
<div class="m-4 mb-2">
  ...
  <select bind:value={selectedState} class="w-1/9 mt-2 p-1 text-sm rounded-md {selectedState ? 'bg-slate-700' : 'bg-slate-600'} text-slate-400">
